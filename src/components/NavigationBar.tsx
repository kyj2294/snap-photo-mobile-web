...
const NavigationBar = () => {
  ...
  return (
    <nav className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 py-2 px-4 fixed bottom-0 w-full flex justify-around items-center z-50">
      <Link to="/" className="flex flex-col items-center">
        <Camera className={`h-6 w-6 ${getActiveClass('/')}`} />
        <span className={`text-xs mt-1 ${getActiveClass('/')}`}>홈</span>
      </Link>
      ...
      <Link to="/rewards" className="flex flex-col items-center">
        ...
        <span className={`text-xs mt-1 ${getActiveClass('/rewards')}`}>리워드</span>
      </Link>
